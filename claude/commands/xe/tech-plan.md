...
- `task-list` 作为可选的二次标准化/补充校验阶段，不再是默认下一步
...
生成内容以 `claude/agents/templates/技术设计文档模板.md` 为准，核心章节包括：
...
12. 详细执行计划（功能任务、接口实现、依赖项、详细接口流程、测试与验收建议）
...
➡️ 下一步：可直接进入实现阶段；如需二次整理任务清单，也可进入 task-list 阶段复核 `docs/{需求名称}/开发任务.md`
```
...
- 数据库相关内容需要根据实际表结构调整
- 本命令结束后会直接输出 `技术设计.md` 与 `开发任务.md`；`task-list` 仅作为可选的再整理阶段
